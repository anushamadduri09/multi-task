...
def parse_example_proto(example_serialized):
  ...
  feature_map = {
      ...
      'image/class/age_label': tf.FixedLenFeature([1], dtype=tf.int64,
                                              default_value=-1),
      'image/class/gender_label': tf.FixedLenFeature([1], dtype=tf.int64,
                                              default_value=-1),
      'image/class/emotion_label': tf.FixedLenFeature([1], dtype=tf.int64,
                                              default_value=-1),
  ...
  age_label = tf.cast(features['image/class/age_label'], dtype=tf.int32)
  gender_label = tf.cast(features['image/class/gender_label'], dtype=tf.int32)
  emotion_label = tf.cast(features['image/class/emotion_label'], dtype=tf.int32)
  return features['image/encoded'], age_label, gender_label, emotion_label, features['image/filename']
...
def batch_inputs(data_dir, batch_size, image_size, train, num_preprocess_threads=4,
  ...
  with tf.name_scope('batch_processing'):
    ...
    for thread_id in range(num_preprocess_threads):
      ...
      image_buffer, age_label_index, gender_label_index, emotion_label_index, fname = parse_example_proto(example_serialized)
          
      image = image_preprocessing(image_buffer, image_size, train, thread_id)
      images_labels_fnames.append([image, age_label_index, gender_label_index, emotion_label_index, fname])

    images, age_label_index_batch, gender_label_index_batch, emotion_label_index_batch, fnames = tf.train.batch_join(
        images_labels_fnames,
    ...
    return images, tf.reshape(age_label_index_batch, [batch_size]), tf.reshape(gender_label_index_batch, [batch_size]), tf.reshape(emotion_label_index_batch, [batch_size]), fnames

def inputs(data_dir, batch_size=128, image_size=227, train=False, num_preprocess_threads=4):
    with tf.device('/cpu:0'):
        images, age_labels, gender_labels, emotion_labels, filenames = batch_inputs(
            data_dir, batch_size, image_size, train,
            num_preprocess_threads=num_preprocess_threads,
            num_readers=1)
    return images, age_labels, gender_labels, emotion_labels, filenames
