...
from data_multitasking import distorted_inputs
from model_multitasking import select_model
import json
import re
...
def optimizer(optim, eta, loss_fn, at_step, decay_rate):
    #this function does the backward pass an updates the model parameters
...
def main(argv=None):
    with tf.Graph().as_default():
        ...
        images, labels_age, labels_gender, labels_emotion, _ = distorted_inputs(FLAGS.train_dir, FLAGS.batch_size, FLAGS.image_size, FLAGS.num_preprocess_threads)
        #forward pass
        logits_age, logits_gender, logits_emotion = model_fn(md['nlabels_age'], md['nlabels_gender'], md['nlabels_emotion'], images, 1-FLAGS.pdrop, True)
        #computing the loss for age label
        loss_age = loss(logits_age, labels_age)
        #computing the loss for gender label
        loss_gender = loss(logits_gender, labels_gender)
        #computing the loss for emotion label
        loss_emotion = loss(logits_emotion, labels_emotion)
        
        #based on flag either multi-task is selected or task-specific learning is selected
        if(int(FLAGS.mloss) == 0):
            print("multi-tasking all")
            total_loss = loss_age + loss_gender + loss_emotion
        elif(int(FLAGS.mloss) == 1):
            print("multi-tasking age and gender")
            total_loss = loss_age + loss_gender
        elif(int(FLAGS.mloss) == 2):
            print("multi-tasking age and emotion")
            total_loss = loss_age + loss_emotion
        elif(int(FLAGS.mloss) == 3):
            print("multi-tasking gender and emotion") 
            total_loss = loss_gender + loss_emotion
        elif(int(FLAGS.mloss) == 4):
            print("age model")
            total_loss = loss_age
        elif(int(FLAGS.mloss) == 5):
            print("gender model")
            total_loss = loss_gender
        elif(int(FLAGS.mloss) == 6):
            print("emotion model")
        ...
        # iterates until numeber steps 
        # the number of steps calcuated based on number of epochs
