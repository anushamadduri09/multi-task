...
def select_model(name):
    '''
    Based on the argument passed model will be selected
    '''
...
def levi_hassner(nlabels_age, nlabels_gender, nlabels_emotion, images, pkeep, is_training):
    '''
    ...
    with tf.variable_scope('output') as scope:
        #the output layer for  age softmax layer   
        weights_age = tf.Variable(tf.random_normal([512, nlabels_age], mean=0.0, stddev=0.01), name='weights')
        biases_age = tf.Variable(tf.constant(0.0, shape=[nlabels_age], dtype=tf.float32), name='biases')
        output_age = tf.add(tf.matmul(drop2, weights_age), biases_age, name=scope.name)
        
        #the output layer for gender softmax layer
        weights_gender = tf.Variable(tf.random_normal([512, nlabels_gender], mean=0.0, stddev=0.01), name='weights')
        biases_gender = tf.Variable(tf.constant(0.0, shape=[nlabels_gender], dtype=tf.float32), name='biases')
        output_gender = tf.add(tf.matmul(drop2, weights_gender), biases_gender, name=scope.name)

        #the output layer for emotion layer
        weights_emotion = tf.Variable(tf.random_normal([512, nlabels_emotion], mean=0.0, stddev=0.01), name='weights')
        biases_emotion = tf.Variable(tf.constant(0.0, shape=[nlabels_emotion], dtype=tf.float32), name='biases')
        output_emotion = tf.add(tf.matmul(drop2, weights_emotion), biases_emotion, name=scope.name)
